  ...
  return {
    async list() {
      return [...byKey.values()]
        ...
        .map((p) => ({ translationKey: p.translationKey, titleDe: p.de.title, slugDe: p.de.slug, slugEn: p.en.slug, status: p.status, updatedAt: p.updatedAt, hasEnBody: Boolean(p.en.bodyMarkdown && p.en.bodyMarkdown.trim()) }));
    },
    async get(tk) {
