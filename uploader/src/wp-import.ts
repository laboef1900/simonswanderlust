...
import { normalizeProcessOptions, type ProcessOptions } from './pipeline.js';
...
export interface ImportDeps {
  ...
  /** Immutable image profile snapshot for this prepared import. */
...
function applyRehosts(body: string, rehosted: ReadonlyMap<string, RehostResult>, images: Record<string, ImageDims>): string {
  ...
  const record = (from: string, r: RehostResult): void => {
    // `format` describes these generated bytes, not editorial metadata. Strip
    // stale source/target markers before applying the current re-host result;
    // omission is the modern AVIF+WebP contract.
    const image: ImageDims = {
      ...images[r.src], ...images[from], width: r.width, height: r.height,
      ...(r.format ? { format: r.format } : {}),
    };
    if (!r.format) delete image.format;
    images[r.src] = image;
...
async function buildLocale(
  ...
  let heroImage: PostLocale['heroImage'] = { ...PLACEHOLDER_HERO };
  const heroUrl = includeHero && p.thumbnailId ? attachments.get(p.thumbnailId) : undefined;
  if (heroUrl) {
    try {
      const r = await rehost(heroUrl, heroKey(p), p.title);
      heroImage = {
        src: r.src, width: r.width, height: r.height, alt: p.title,
        ...(r.format ? { format: r.format } : {}),
      };
...
export async function prepareImport(xml: string, deps: ImportDeps): Promise<PreparedImport> {
  ...
  const encoding = normalizeProcessOptions(deps.encoding);
  const baseRehost = deps.rehost ?? ((url, key, alt) => rehostImage(url, key, alt, {
    storageDir: deps.storageDir, baseUrl: deps.baseUrl, lock: deps.lock, encoding,
  }));
