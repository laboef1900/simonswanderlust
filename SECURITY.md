...
- **Passwords** are hashed with **scrypt** (`N=16384, r=8, p=1`, 64-byte key, per-user random salt)
  ...
  one rule with no way around it (#109). **Usernames** are validated at creation (`/setup`,
  `POST /users`) by `usernamePolicyViolation`: 1–64 characters from `[A-Za-z0-9._-]` (#131) —
  plain ASCII identifiers, so no markup, whitespace, or homoglyph can become a limiter key, a
  `lower(username)` lookup, an `aria-label`, or a log line. `/login` and the CLI still accept
  any stored name, so an account that predates the rule keeps signing in. The admin shell in
  `auth.js` sets the username into the sidebar badge with `textContent`, never `innerHTML`.
